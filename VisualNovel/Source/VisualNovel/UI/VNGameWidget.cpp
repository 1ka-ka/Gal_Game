...
void UVNGameWidget::BuildWidgetTree()
{
	...
	ContinuePromptText = WidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass());
	ContinuePromptText->SetText(FText::FromString(TEXT("▼")));
	FSlateFontInfo ClickFont = ContinuePromptText->GetFont();
	ClickFont.Size = 16;
	ContinuePromptText->SetFont(ClickFont);
	ContinuePromptText->SetColorAndOpacity(FSlateColor(FLinearColor(1.0f, 1.0f, 1.0f, 0.6f)));
	ContinuePromptText->SetJustification(ETextJustify::Center);
	UCanvasPanelSlot* PromptSlot = DlgPanel->AddChildToCanvas(ContinuePromptText);
	PromptSlot->SetAnchors(FAnchors(0.85f, 0.75f, 0.95f, 0.95f));
	PromptSlot->SetOffsets(FMargin(0));
	PromptSlot->SetZOrder(11);
