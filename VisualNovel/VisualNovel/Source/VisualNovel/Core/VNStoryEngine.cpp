...
bool UVNStoryEngine::LoadStory(const FString& StoryId)
{
	...
	UE_LOG(LogTemp, Log, TEXT("VNStoryEngine: Story file loaded, size=%d bytes"), JsonString.Len());
	...
	{
		UE_LOG(LogTemp, Error, TEXT("VNStoryEngine: Failed to parse story JSON: %s"), *StoryId);
		return false;
	}
